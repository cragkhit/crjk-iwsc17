...
public class FailOnTimeout extends Statement {
    ...
    public void evaluate() throws Throwable {
        ...
        final Thread thread = new Thread ( threadGroup, task, "Time-limited test" );
    ...
    private Thread getStuckThread ( final Thread mainThread ) {
        final List<Thread> threadsInGroup = this.getThreadsInGroup ( mainThread.getThreadGroup() );
    ...
    private List<Thread> getThreadsInGroup ( final ThreadGroup group ) {
        final int activeThreadCount = group.activeCount();
        int threadArraySize = Math.max ( activeThreadCount * 2, 100 );
        for ( int loopCount = 0; loopCount < 5; ++loopCount ) {
            final Thread[] threads = new Thread[threadArraySize];
