...
public class Assert {
    ...
    public static <T> void assertThat ( final String reason, final T actual, final Matcher<? super T> matcher ) {
        MatcherAssert.assertThat ( reason, ( Object ) actual, ( Matcher ) matcher );
    }
    public static void assertThrows ( final Class<? extends Throwable> expectedThrowable, final ThrowingRunnable runnable ) {
    ...
    public static <T extends Throwable> T expectThrows ( final Class<T> expectedThrowable, final ThrowingRunnable runnable ) {
        try {
            ...
            final String mismatchMessage = format ( "unexpected exception type thrown;", expectedThrowable.getSimpleName(), actualThrown.getClass().getSimpleName() );
            final AssertionError assertionError = new AssertionError ( ( Object ) mismatchMessage );
            assertionError.initCause ( actualThrown );
            throw assertionError;
        }
        final String message = String.format ( "expected %s to be thrown, but nothing was thrown", expectedThrowable.getSimpleName() );
        throw new AssertionError ( ( Object ) message );
    }
}
